...
Ingestion persists incident facts and remains the detector for matching/malformed conditions using the existing `INGEST_*` settings. Telegram delivery belongs only to `telegram-ops`; its independent freshness watchdog is the sole Telegram notifier for `ingest.stale`, preventing duplicate worker/watchdog ACTIVE/RECOVERY delivery while still detecting a stopped worker. With one-shot spool shedding as the explicit exception, Telegram delivers worker-owned incidents only after the persisted occurrence count reaches `INGEST_ALERT_FAILURE_THRESHOLD`. Normal delivery markers are PostgreSQL-durable. PostgreSQL-outage ACTIVE/RECOVERY and command-unavailable fallback state are necessarily bounded and process-local while the database itself is unavailable, so exactly-once delivery is not claimed across a bot restart during that outage.
...
Pilot reporting accepts both compact line inputs such as `C1` and punctuated variants such as `C-1`, preferring compact public codes in output. Telegram uses safe HTML with escaped dynamic values, semantic status emoji plus text, whitespace and human-readable metrics instead of raw row JSON. Worst-line and worst-station Telegram rankings require respectively 100 and 30 usable observations so tiny samples are reported as insufficient rather than presented as meaningful extremes.
