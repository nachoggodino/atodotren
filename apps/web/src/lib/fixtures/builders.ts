...
export function liveMeta(scenario: FixtureScenario, stats: SummaryStats, activeTrains: number): ResponseMeta {
  ...
  return liveResponseMeta({ stats, sourceAt, activeTrains, serviceDate: FIXTURE_TODAY, finalization: scenario === "finalized" ? { state: "finalized", finalizedAt: "2026-08-25T02:10:00.000Z" } : { state: "processing", finalizedAt: null }, provenance: fixtureProvenance(scenario), precision: scenario === "overnight" ? "reported" : "mixed", expectedOvernight: scenario === "overnight", cache: scenario === "offline-cached" ? "offline-cache" : "origin", now: new Date(FIXTURE_NOW) });
}

export function historyMeta(scenario: FixtureScenario, stats: SummaryStats, filters: HistoryFilters): ResponseMeta {
  const finalization = scenario === "finalized" ? { state: "finalized" as const, finalizedAt: "2026-08-25T02:10:00.000Z" } : filters.to === FIXTURE_TODAY ? { state: "processing" as const, finalizedAt: null } : { state: "unknown" as const, finalizedAt: null };
  return historicalResponseMeta({ stats, serviceDate: filters.from === filters.to ? filters.to : null, finalization, provenance: fixtureProvenance(scenario), cache: scenario === "offline-cached" ? "offline-cache" : "origin", now: new Date(FIXTURE_NOW) });
}
...
export function lineStats(index: number, scenario: FixtureScenario): SummaryStats {
  ...
  return statsForScenario({ scheduled, observed, punctuality: Math.max(0.48, 0.84 - index * 0.035), meanDelaySeconds: 118 + index * 32, medianDelaySeconds: 90 + index * 30, p90DelaySeconds: 330 + index * 45, canceled, missing: scheduled - observed, distribution: distributionFromCounts({ early: 12, punctual: Math.round(observed * 0.65), "delay-2-5": Math.round(observed * 0.17), "delay-5-10": Math.round(observed * 0.09), "delay-10-15": Math.round(observed * 0.05), "delay-15-plus": Math.round(observed * 0.04) }) }, scenario);
}

export function linePerformance(scenario: FixtureScenario): readonly LinePerformance[] {
  return fixtureLines.map((line, index) => ({ ...line, stats: scenario === "unsupported-capabilities" ? { status: "unavailable", reason: "not-provided" } : { status: "available", value: lineStats(index, scenario) }, activeTrains: scenario === "overnight" || scenario === "outage" ? 0 : Math.max(1, 12 - index) }));
}
...
function pattern(id: string, branchSlug: string, direction: 0 | 1, stationIndexes: readonly number[]): SchematicPattern {
  ...
  return { id, branchSlug, direction: { id: direction, headsign: null, from: selected[0] ?? null, to: selected.at(-1) ?? null }, stops: selected.map((station, index) => ({ station, order: index + 1 })), destination: selected.at(-1) ?? null };
}
...
function train(input: { readonly id: string; readonly pattern: SchematicPattern | null; readonly position: TrainDetail["position"]; readonly delaySeconds: number | null; readonly state: EvidenceState }): TrainDetail {
  ...
  return { id: input.id, journeyId: `journey-${input.id}`, serviceDate: FIXTURE_TODAY, line, patternId: input.pattern?.id ?? null, direction: input.pattern?.direction ?? null, headsign: input.pattern?.destination?.name ?? null, destination: input.pattern?.destination ?? null, currentStation, previousStation, nextStation, position: input.position, scheduledArrivalAt: scheduled, probableArrivalAt: probable, renfeReportedArrivalAt: reportedArrival, observedPresenceAt: input.state === "observed_presence" ? FIXTURE_NOW : null, observedPastArrivalAt: previousStation === null ? null : "2026-08-24T19:58:30.000Z", delaySeconds: input.delaySeconds, state: input.state, sourceAt: "2026-08-24T19:59:42.000Z", information: { servicePattern: input.pattern === null ? unavailable : inferredHigh, direction: input.pattern === null ? unavailable : inferredHigh, headsign: input.pattern?.destination === null || input.pattern?.destination === undefined ? unavailable : reportedHigh, destination: input.pattern?.destination === null || input.pattern?.destination === undefined ? unavailable : inferredHigh, currentStation: currentStation === null ? unavailable : reportedHigh, previousStation: previousStation === null ? unavailable : inferredMedium, nextStation: nextStation === null ? unavailable : inferredMedium, scheduledArrival: reportedHigh, probableArrival: probable === null ? unavailable : inferredMedium, reportedArrival: reportedArrival === null ? unavailable : reportedHigh, observedPresence: input.state === "observed_presence" || previousStation !== null ? reportedHigh : unavailable, delay: input.delaySeconds === null ? unavailable : reportedHigh } };
}
...
function fixtureInsights(kind: "network" | "line" | "station", scenario: FixtureScenario): HistoryResponse["insights"] {
  ...
  const hourWeekday: HourWeekdayItem[] = Array.from({ length: 7 * 24 }, (_, index) => { const weekday = index % 7; const hour = Math.floor(index / 7); const scheduled = 70 + ((weekday * 17 + hour * 13) % 80); const sample = Math.round(scheduled * (0.78 + ((weekday + hour) % 5) * 0.04)); return { weekday, hour, scheduled, sample, meanDelaySeconds: 90 + ((weekday * 41 + hour * 23) % 520), coverage: sample / scheduled }; });
  const volumeReliability: VolumeReliabilityItem[] = kind === "network" ? fixtureLines.slice(0, 8).map((line, index) => ({ ...rankingItem(line.id, line.code, index + 1), scheduled: 230 + index * 54, coverage: 0.82 + (index % 4) * 0.035 })) : stationValues.map((item, index) => ({ ...item, scheduled: item.sample + 30 + index * 7, coverage: item.sample / (item.sample + 30 + index * 7) }));
  const segments: SegmentDelayItem[] = kind === "line" ? fixtureStations.slice(0, 5).flatMap((station, index) => { const next = fixtureStations[index + 1]; if (next === undefined) return []; return [0, 1].map((direction) => ({ id: `${station.id}-${next.id}-${direction}`, label: `${station.name.es} → ${next.name.es}`, direction: direction as 0 | 1, sample: 180 + index * 23, addedDelaySeconds: 18 + index * 21 + direction * 9 })); }) : [];
  ...
  return { stations: kind === "station" ? { status: "unavailable", reason: "not-supported" } : available(stationValues), hours: available(hourValues), hourWeekday: available(hourWeekday), volumeReliability: available(volumeReliability), segments: kind === "line" ? available(segments) : { status: "unavailable", reason: "not-supported" }, scheduleSlots: { status: "unavailable", reason: "not-supported" } };
}
...
export function historyResponse(filters: HistoryFilters, kind: "network" | "line" | "station", label: string, id: string, slug: HistoryResponse["context"]["slug"], seed: number, scenario: FixtureScenario): HistoryResponse {
  ...
  return { meta: historyMeta(scenario, stats, filters), context: { kind, label, id: kind === "network" ? MADRID_NETWORK.slug : id, slug }, filters, stats, trend, rankings, insights: fixtureInsights(kind, scenario), directions: [{ id: 0, headsign: null, from: kind === "line" ? fixtureStations[0]! : null, to: kind === "line" ? fixtureStations[5]! : null }, { id: 1, headsign: null, from: kind === "line" ? fixtureStations[5]! : null, to: kind === "line" ? fixtureStations[0]! : null }] };
}
...
function syntheticStation(index: number) { return { id: `synthetic-${index}`, slug: { es: `estacion-sintetica-${index}`, en: `synthetic-station-${index}` }, name: { es: `Estación sintética ${index}`, en: `Synthetic station ${index}` } }; }
...
export function matrixResponse(scenario: FixtureScenario): MatrixResponse {
  ...
  const journeys: MatrixJourney[] = Array.from({ length: journeyCount }, (_, index) => ({ id: `mx-${index + 1}`, label: `C1 ${String(700 + index * 13)}`, direction: { id: index % 2 === 0 ? 0 : 1, headsign: null, from: null, to: null } }));
