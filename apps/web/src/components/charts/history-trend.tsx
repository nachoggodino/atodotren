...
export function HistoryTrend({ points, lang, messages }: { readonly points: readonly HistoryPoint[]; readonly lang: Lang; readonly messages: Messages }) {
  ...
  return <div>
    <div style={{ height: CHART_LAYOUT.trendHeight }} role="img" aria-label={messages.charts.meanDelayTrend}><ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={CHART_LAYOUT.margin}><CartesianGrid vertical={false} stroke="var(--border)" /><XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: "var(--muted)", fontSize: 11 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: "var(--muted)", fontSize: 11 }} width={42} unit="m" /><Tooltip contentStyle={{ background: "var(--surface-strong)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--foreground)" }} /><Line type="monotone" dataKey="delay" stroke="var(--primary)" strokeWidth={3} dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div>
    <table className="sr-only"><caption>{messages.charts.accessibleData}: {messages.charts.meanDelayTrend}</caption><thead><tr><th scope="col">{messages.charts.date}</th><th scope="col">{messages.charts.value}</th></tr></thead><tbody>{data.map((item) => <tr key={item.rawDate}><th scope="row">{item.rawDate}</th><td>{item.delay === null ? messages.common.noData : `${item.delay} min`}</td></tr>)}</tbody></table>
  </div>;
}
