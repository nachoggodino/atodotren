...
interface PlottedStop extends SchematicStop { readonly x: number; readonly y: number }

function layoutStops(stops: readonly SchematicStop[]): readonly PlottedStop[] {
  return [...stops]
    .sort((left, right) => left.order - right.order)
    .map((stop, index) => ({ ...stop, x: 70 + index * 105, y: index % 3 === 1 ? 130 : 108 }));
}

function trainPoint(train: TrainDetail, stops: readonly PlottedStop[]): { x: number; y: number } | null {
...
export function SchematicMap({ stops, trains, lineColor, lang, messages }: { readonly stops: readonly SchematicStop[]; readonly trains: readonly TrainDetail[]; readonly lineColor: string; readonly lang: Lang; readonly messages: Messages }) {
  ...
  const plottedStops = useMemo(() => layoutStops(stops), [stops]);
  const width = Math.max(720, (plottedStops.at(-1)?.x ?? 680) + 90);
  const plottedTrains = useMemo(() => trains.map((train) => ({ train, point: trainPoint(train, plottedStops) })).filter((item): item is { train: TrainDetail; point: { x: number; y: number } } => item.point !== null), [trains, plottedStops]);
  return <div>
    <div className="overflow-x-auto rounded-xl border border-border bg-surface-strong" data-testid="schematic-map"><svg aria-label={messages.live.schematic} className="min-h-[240px]" role="group" viewBox={`0 0 ${width} 250`} width={width} height="250">
      <path d={plottedStops.map((stop, index) => `${index === 0 ? "M" : "L"}${stop.x} ${stop.y}`).join(" ")} fill="none" stroke={lineColor} strokeLinecap="round" strokeLinejoin="round" strokeWidth="10" opacity=".86" />
      {plottedStops.map((stop) => <g key={stop.station.id}><circle cx={stop.x} cy={stop.y} r="8" fill="var(--surface-strong)" stroke={lineColor} strokeWidth="4" /><text x={stop.x} y={stop.y + 29} textAnchor="middle" fill="var(--foreground)" fontSize="11" fontWeight="700">{stop.station.name[lang].slice(0, 20)}</text></g>)}
