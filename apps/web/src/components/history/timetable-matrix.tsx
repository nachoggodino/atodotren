...
export function TimetableMatrix({ matrix, lang, messages }: { readonly matrix: MatrixResponse; readonly lang: Lang; readonly messages: Messages }) {
  ...
  return <div>
    <div className="mb-3 flex flex-wrap gap-x-4 gap-y-2 text-xs text-muted" aria-label={messages.history.legend}><span>✓ ≤2m</span><span>+ 2–10m</span><span>!! &gt;10m</span><span>× {messages.history.canceled}</span><span>↷ {messages.history.skipped}</span><span>— {messages.common.missing.toLowerCase()}</span><span>… {messages.history.pending}</span></div>
    <div className="max-h-[70vh] overflow-auto rounded-xl border border-border" data-testid="timetable-matrix"><table className="min-w-max border-collapse text-xs"><thead className="sticky top-0 z-20 bg-surface-strong"><tr><th className="sticky left-0 z-30 min-w-44 border-b border-r border-border bg-surface-strong p-3 text-left">{messages.history.stationColumn}</th>{matrix.journeys.map((journey) => <th className="min-w-24 border-b border-border p-2 text-center font-bold" key={journey.id}>{journey.label}</th>)}</tr></thead><tbody>{matrix.stations.map((station) => <tr key={station.id}><th className="sticky left-0 z-10 max-w-64 border-r border-t border-border bg-surface-strong p-3 text-left font-bold whitespace-normal">{station.name[lang]}</th>{matrix.journeys.map((journey) => { const cell = cells.get(`${station.id}:${journey.id}`); if (cell === undefined) return <td className="border-t border-border p-2 text-center text-muted" key={journey.id}>·</td>; return <td className="border-t border-border p-0" key={journey.id}><button className="delay-cell grid min-h-14 w-full place-items-center gap-0.5 px-2 py-1 text-center hover:ring-2 hover:ring-inset hover:ring-primary" data-band={delayBand(cell.delaySeconds)} data-state={cell.state} onClick={() => setSelected(cell)} type="button"><strong className="font-mono text-[11px]">{formatMadridTime(cell.scheduledAt, lang)}</strong><span aria-hidden="true" className="font-black">{symbol(cell)}</span><span className="sr-only">{evidenceLabel(cell.state, messages)}, {formatDelay(cell.delaySeconds, lang)}</span></button></td>; })}</tr>)}</tbody></table></div>
    {selected ? <div className="mt-3 flex items-start justify-between gap-4 rounded-lg border border-border bg-surface-strong p-4" data-testid="matrix-detail"><div><p className="eyebrow">{messages.common.details}</p><p className="mt-1 font-bold">{formatMadridTime(selected.scheduledAt, lang)} · {evidenceLabel(selected.state, messages)}</p><p className="mt-1 text-sm text-muted">{formatDelay(selected.delaySeconds, lang)}</p></div><button className="grid size-10 place-items-center rounded-md hover:bg-muted-soft" onClick={() => setSelected(null)} aria-label={messages.nav.close} type="button"><X className="size-4" /></button></div> : null}
  </div>;
}
