...
import { StationLinePunctualityChart } from "@/components/charts/station-line-punctuality";
import { LivePageSummary } from "@/components/live/live-page-summary";
import { StationRegularity } from "@/components/live/station-regularity";
  ...
  return (
    <div className="page-shell pb-20 pt-7 sm:pt-9">
      ...
      <section className="mt-12 grid gap-10 border-t border-border pt-8 lg:grid-cols-2">
        <div><h2 className="text-2xl font-black">{messages.live.stationLinePunctuality}</h2><div className="mt-4"><StationLinePunctualityChart messages={messages} values={data.stationInsights.linePunctuality} /></div></div>
        <div><h2 className="text-2xl font-black">{messages.live.stationRegularity}</h2><div className="mt-4"><StationRegularity lang={lang} messages={messages} value={data.stationInsights.cadence} /></div></div>
      </section>
    </div>
  );
}
