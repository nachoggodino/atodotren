...
export default async function LiveLinePage({ params, searchParams }: { readonly params: Promise<{ lang: string; slug: string }>; readonly searchParams: Promise<{ scenario?: string }> }) {
  ...
  return <div className="page-shell pb-20 pt-12"><p className="eyebrow">{messages.live.title} · {data.context.code}</p><h1 className="mt-3 text-4xl font-black tracking-[-.05em] sm:text-6xl">{data.context.name[lang]}</h1><div className="mt-6"><DataMeta meta={data.meta} lang={lang} messages={messages} /><LiveRefresh messages={messages} /></div><div className="mt-8"><StatsBand stats={data.stats} lang={lang} messages={messages} /></div><section className="mt-12"><div className="mb-4 flex items-baseline justify-between gap-4"><h2 className="text-2xl font-black">{messages.live.schematic}</h2><span className="text-sm text-muted">{data.trains.length} {messages.live.activeTrains}</span></div>{data.stops.length === 0 ? <p className="border-y border-border py-8 text-muted">{messages.common.noData}</p> : <SchematicMap stops={data.stops} trains={data.trains} lineColor={data.context.color} lang={lang} messages={messages} />}</section><section className="mt-12 grid gap-8 border-t border-border pt-8 lg:grid-cols-2"><div><h2 className="text-2xl font-black">{messages.history.distribution}</h2><DelayDistribution values={data.stats.distribution} lang={lang} messages={messages} /></div><div><p className="eyebrow">{messages.live.comparison}</p><p className="metric-value mt-3 text-5xl font-black">{data.comparison.meanDelaySeconds === null ? "—" : `${Math.round(data.comparison.meanDelaySeconds / 60)} min`}</p><p className="mt-2 text-sm text-muted">{messages.live.comparisonDescription}</p></div></section></div>;
}
