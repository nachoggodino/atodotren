  ...
  return <div className="page-shell overflow-x-clip pb-6 pt-9 sm:pb-8 sm:pt-14">
    ...
    <section className="mt-9 overflow-hidden rounded-2xl border border-border bg-surface-strong" data-testid="landing-live-metrics">
      <div className="grid grid-cols-3 divide-x divide-border">
        <div className="flex min-h-[5.75rem] min-w-0 flex-col justify-center gap-1.5 px-3 py-3 sm:min-h-24 sm:px-5" style={{ background: "color-mix(in srgb, var(--landing-delay) 4%, var(--surface-strong))" }}><p className="text-[.5rem] font-bold uppercase leading-tight tracking-[.07em] text-muted sm:text-[.55rem]">{messages.landing.activeDelay}</p><p className="metric-value text-[.94rem] font-black text-[var(--landing-delay)] sm:text-[1.18rem]">{formatDuration(data.activeDelaySeconds, lang)}</p></div>
        <div className="flex min-h-[5.75rem] min-w-0 flex-col justify-center gap-1.5 px-3 py-3 sm:min-h-24 sm:px-5" style={{ background: "color-mix(in srgb, var(--landing-positive) 6%, var(--surface-strong))" }}><p className="text-[.5rem] font-bold uppercase leading-tight tracking-[.07em] text-muted sm:text-[.55rem]">{messages.landing.activeTrains}</p><p className="metric-value text-center text-[29px] font-black leading-none text-[var(--landing-positive)] sm:text-[33px]" data-testid="landing-active-trains-value">{number.format(data.activeTrains)}</p></div>
        <div className="flex min-h-[5.75rem] min-w-0 flex-col justify-center gap-1.5 px-3 py-3 sm:min-h-24 sm:px-5" style={{ background: "color-mix(in srgb, var(--landing-delay) 6%, var(--surface-strong))" }}><p className="text-[.5rem] font-bold uppercase leading-tight tracking-[.07em] text-muted sm:text-[.55rem]">{messages.landing.todayDelay}</p><p className="metric-value text-[.94rem] font-black text-[var(--landing-delay)] sm:text-[1.18rem]">{formatDuration(data.dayDelaySeconds, lang)}</p></div>
      </div>
    </section>
