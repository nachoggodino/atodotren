...
export default async function LandingPage({ params, searchParams }: { readonly params: Promise<{ lang: string }>; readonly searchParams: Promise<{ scenario?: string }> }) {
  ...
  return <div className="page-shell overflow-x-clip pb-6 pt-9 sm:pb-10 sm:pt-14">
    ...
    <footer className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-border pt-5 text-sm text-muted"><span>{BRAND.name} · {messages.landing.footerDescription}</span><Link className="flex items-center gap-1 font-bold text-foreground" href={`/${lang}/methodology`}>{messages.landing.methodology}<ArrowUpRight className="size-4" /></Link></footer>
  </div>;
}
